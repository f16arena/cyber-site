// CS2 map pool для F16 Arena (Active Duty + ротация).
// id — то, что хранится в БД (Match.map, MatchVetoAction.map).
...
export const CS2_MAP_POOL: ReadonlyArray<Cs2MapInfo> = [
  {
    id: "mirage",
    displayName: "Mirage",
    csName: "de_mirage",
...
export const CS2_MAP_BY_ID = new Map<string, Cs2MapInfo>(
  CS2_MAP_POOL.map((m) => [m.id, m])
);

export function isCs2MapId(value: string): value is Cs2MapId {
  return CS2_MAP_BY_ID.has(value);
}

export function csNameFor(mapId: string): string | null {
  return CS2_MAP_BY_ID.get(mapId)?.csName ?? null;
}

export function displayNameFor(mapId: string): string {
  return CS2_MAP_BY_ID.get(mapId)?.displayName ?? mapId;
}
